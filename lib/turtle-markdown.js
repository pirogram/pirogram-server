...
function commonmarkToHtml( markdown) {
...
function textToMcQuiz( id, markdown) {
    ...
    for( line of lines) {
        ...
        if( !reOption.exec(line)) {
            ...
            option.html = commonmarkToHtml(option.text);
    ...
    return {id, text, html: commonmarkToHtml(text), options, markdown};
}
...
function textToRegexQuiz( id, markdown) {
    ...
    const text = textLines.join('\n');
    return {id, text, html: commonmarkToHtml(text), solution, options, markdown};
}
...
function textToOfflineExercise( id, markdown) {
    ...
    return {id, text: text, html:commonmarkToHtml(text), markdown, solution, solutionHtml: commonmarkToHtml(solution)};
}
...
function parseMarkdown( markdown) {
    ...
    if( results.length === 0) {
        return [{id: uuidv4(), type: 'text', html: commonmarkToHtml(markdown), markdown: markdown}];
    }
    ...
    for( result of results) {
        ...
        const html = commonmarkToHtml( md);
...
module.exports = { parseMarkdown, commonmarkToHtml, convertQuizContent};