...
module.exports = (path, opts) => {
    ...
    env.addFilter('markdown', function(str) {
        return new nunjucks.runtime.SafeString(markdownToHtml(str));
    });

    env.addFilter('datetime', function(d) {
        return d ? new nunjucks.runtime.SafeString( d.toLocaleString('en-US')) : ''
    })
