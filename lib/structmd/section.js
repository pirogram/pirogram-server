...
import {Content, MarkdownContent, CodeExplorerContent, 
...
export class Section {
    ...
    getRawContent() {
        while( this.rawContentLines.length && this.rawContentLines[this.rawContentLines.length-1].trim() == '') {
            this.rawContentLines.splice(this.rawContentLines.length-1, 1);
        }

        return this.rawContentLines.join('\n');
    }
...
function buildCodeContent(s) {
    const c = new CodeExplorerContent();

    copyId(s, c);
    c.starterCode = s.getRawContent();
    c.lang = s.meta['lang'] || c.lang;
    c.chained = s.meta['chained'] == 'true' ? true : false;
...
export function buildMultipleChoice(s) {
    ...
    c.question = s.children[0].getRawContent();

    if(s.children[1].name == 'code') {
        c.code = s.children[1].getRawContent();
    }
...
function buildCodingProblemContent(s) {
    ...
    c.problemStatement = markdownSection.getRawContent();

    c.starterCode = getSectionByName(s.children, 'starter-code').getRawContent();
    c.referenceSolution = getSectionByName(s.children, 'solution').getRawContent();
    c.tests = getSectionByName(s.children, 'tests').getRawContent();
...
function buildTextFibContent(s) {
    ...
    let markdown = s.children[0].getRawContent();
    c.markdown = markdown;
