...
export class CodeExecutor {
    ...
    static buildPool() {
        if( available.length >= POOL_LOW_WATERMARK) return;

        while(available.length < POOL_HIGH_WATERMARK) {
    ...
    async initCode() {
        await this.execute(`import os\nos.chdir('/home/jupyter/content')\nimport matplotlib.pyplot\n`);
    }
...
StateMachine.factory(CodeExecutor, {
    ...
    methods: {
        ...
        onEnterIdling: function () {
            ...
            if(queuedItem) {
                /* Let the current state transition happen and then move to next request. */
                setTimeout(() => {
                    this.executionBegin(queuedItem.code, queuedItem.resolve, queuedItem.reject);
                }, 100);
            }
        },
