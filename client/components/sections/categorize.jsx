...
export default class CategorizationQuestion extends React.Component {
    ...
    render() {
        ...
        const dropdownValues = this.props.categories.map( (category, index) => {
        ...
        return (
            <Segment id={this.props.id} className='exercise'>
                ...
                <Form className={this.state.serverError ? "quiz error" : "quiz"}
                    onSubmit={(e) => { 
                        e.preventDefault();
                        categorizationAnswerCheck( this.props.id);
                    }}>
                        ...
                        {this.props.challenges.map( (challenge, index) => {
                            ...
                            return <Form.Field width={8} key={index} className={className}>
                                        ...
                                        <Dropdown placeholder='Select' selection options={dropdownValues}
                                            ...
                                            onChange={(e, data) => {
                                                dispatch('CATEGORY_SELECTED', {exerciseId: this.props.id, challenge: challenge, category: data.value});
                                            }}/>
                                        {this.state.showHint && 
                                            this.state.selectedCategories[challenge] != 
