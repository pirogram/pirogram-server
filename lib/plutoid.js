...
async function createSession() {
    ...
    const args = ['run', '-p', `${port}:6699/tcp`, '-d', '-t', '--log-driver', 'fluentd',
        '--mount', `type=bind,source=${datasetDir},destination=/data,readonly`,
        '--log-opt', `tag=docker.plutoid-${port}`, 'manasgarg/plutoid', 'plutoidweb'];
    await exec( cmd, args);
