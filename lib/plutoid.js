...
function handleMultiline( tests) {
    const cleanedTests = [];
    tests.map( (test, i) => {
        if( test[0] == ' ' || test[0] == '\t') {
            cleanedTests[ cleanedTests.length - 1] = cleanedTests[ cleanedTests.length - 1] + test;
        } else {
            cleanedTests.push( test);
        }
    });
